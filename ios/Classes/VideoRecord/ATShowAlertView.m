...
@implementation ATShowAlertView
...
-(void)showPermissionSetting:(NSString*)alertMessage{
    ...
    UIAlertAction* settingAction = [UIAlertAction actionWithTitle:@"前往设置" style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction * action) {
                                                              if (self.defaultlAction) {
                                                                  self.defaultlAction();
                                                              }
        ...
        UIAlertAction* nextAction = [UIAlertAction actionWithTitle:@"下次再说" style:UIAlertActionStyleCancel
                                                             handler:^(UIAlertAction * action) {
                                                                 if (self.subTitleAction) {
                                                                     self.subTitleAction();
                                                                 }
...
-(void)requesetVideoPermissionCompletion:(void(^)(BOOL permission))permission{
    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        ...
        permission(granted);
        if (!granted) {
            [self showPermissionSetting:@"请在iPhone的“设置-隐私”选项中，允许访问你的相册"];
        }
    } ];
    ...
    if ([audioSession respondsToSelector:@selector(requestRecordPermission:)]) {
        [audioSession performSelector:@selector(requestRecordPermission:) withObject:^(BOOL granted) {
            if (granted) {
                ...
                bCanRecord = NO;
                [self showPermissionSetting:@"请在iPhone的“设置-隐私”选项中，允许访问你的麦克风"];
            }
        }];
    }
    //    }
